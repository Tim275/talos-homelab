...
```bash
# Manuell triggern (wartet sonst auf 2h-Schedule)
kubectl create job --from=cronjob/renovate-bot -n renovate manual-test-run

# Logs verfolgen
kubectl logs job/manual-test-run -n renovate -f
